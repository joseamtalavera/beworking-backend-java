...
public class InvoicePdfService {
    ...
    // Brand colors
    private static final Color BRAND_GREEN = new Color(0, 150, 36);
    private static final Color BRAND_GREEN_DARK = new Color(0, 122, 29);
    private static final Color BRAND_GREEN_LIGHT = new Color(46, 204, 113);
    private static final Color INK = new Color(26, 26, 26);
    private static final Color MUTED = new Color(113, 113, 122);
    private static final Color BORDER = new Color(229, 229, 229);
    ...
    record CompanyInfo(String name, String address, String cityLine, String email, String cif) { }

    static final CompanyInfo COMPANY_PT = new CompanyInfo(
    ...
    static final CompanyInfo COMPANY_GT = new CompanyInfo(
        "GLOBALTECHNO O\u00dc",
        "Sepapaja tn 6",
    ...
    public InvoicePdfService(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    /**
     * Generate a PDF for the given invoice ID. Returns the PDF bytes, or null if the invoice is not found.
     */
    ...
    /**
     * Returns the display number for the invoice, useful for filenames.
     */
    public String getDisplayNumber(Long invoiceId) {
        InvoiceHeader header = fetchHeader(invoiceId);
        return header != null ? header.displayNumber() : invoiceId.toString();
    }

    /**
     * Returns the client email for the given invoice.
     */
    ...
    /**
     * Returns the client name for the given invoice.
     */
    ...
    /**
     * Returns the total amount for the given invoice.
     */
    public BigDecimal getInvoiceTotal(Long invoiceId) {
        InvoiceHeader header = fetchHeader(invoiceId);
        return header != null ? header.total() : null;
    }
    ...
    String fetchCenterName(Integer centerId) {
    ...
    private void drawInvoice(PDDocument doc,
        ...
        try {
            ...
            float summaryReserve = 200f; // increased for bank details
            ...
            // Ensure space for summary + bank details
    ...
    private float drawHeaderSection(PDPageContentStream cs,
        ...
        float cursorY = height - margin - 10;
        ...
        companyY -= 11;
        addTextRightAligned(cs, PDType1Font.HELVETICA, 8.5f, companyX, companyY, company.email() + " \u00b7 " + company.cif());
    ...
    private static void addTextRightAligned(PDPageContentStream cs, PDType1Font font, float fontSize, float x, float y, String text) throws IOException {
    ...
    private static String buildClientBlock(ClientInfo info) {
        ...
        if (info.name() != null && !info.name().isBlank()) parts.add(info.name());
        if (info.email() != null && !info.email().isBlank()) parts.add(info.email());
        List<String> addressPieces = new ArrayList<>();
        if (info.address() != null && !info.address().isBlank()) addressPieces.add(info.address());
        List<String> locality = new ArrayList<>();
        if (info.postalCode() != null && !info.postalCode().isBlank()) locality.add(info.postalCode());
        if (info.city() != null && !info.city().isBlank()) locality.add(info.city());
        if (info.province() != null && !info.province().isBlank()) locality.add(info.province());
        if (!locality.isEmpty()) addressPieces.add(String.join(" \u00b7 ", locality));
        if (info.country() != null && !info.country().isBlank()) addressPieces.add(info.country());
    ...
    record LineItem(String concept, BigDecimal quantity, BigDecimal unitPrice, BigDecimal total) { }

    record ClientInfo(String name, String email, String address, String postalCode,
                      String city, String province, String country, String taxId) { }
}
